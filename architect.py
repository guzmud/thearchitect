...
def load_aliases():
    ...
    for i in filter(None, data.split("#DEF")):
        tdict[filter(None, i.split("#ALIAS"))[0].strip().split("[")[0]] = [
            filter(None,
                   no_comments(filter(None,
                                      i.split("#ALIAS"))[0].split("\n")))[0].strip(),
            filter(None,
                   no_comments(filter(None,
                                      i.split("#ALIAS"))[1].split("\n")))]
...
def format_args(strarg):
    ...
    # switching reserved chars inside
    # instruction injections to avoid parser confusing
...
def code_engine(plist):
    ...
    tf = open(str(cconf.pieces_folder)+"/"+str(inst)+'.piece',
              'r')
    temp = tf.read()
    tf.close()
        ...
        if plist[i][0] in alias_dict.keys():
            ...
            for j in range(len(at_list)):
                ...
                for k in range(len(iarglist)):
                    at_list[j] = [at_list[j][0],
                                  at_list[j][1].replace("IVAR"+str(k+1), iarglist[k]),
                                  at_list[j][2]]
                ...
                for q in range(len(oarglist)):
                    at_list[j] = [at_list[j][0],
                                  at_list[j][1],
                                  at_list[j][2].replace("OVAR"+str(q+1), oarglist[q])]
...
def pparts_process(pparts):
    ...
    for part in pparts:
        ...
        # switching reserved chars inside instruction injections
        # in order to avoid parser confusing
        ...
        plist += [[part.split('[')[0],
                   ...
                   part.split('[')[2].split(']')[0]], ]
    return plist


def preprocess_schematic(schmdata):
    sample_parts = filter(None,
                          map(str.strip,
                              schmdata.split("#SAMPLE")[1].split("#PIECES")[0].split("\n"))
                          )
    pieces_parts = filter(None,
                          map(str.strip,
                              schmdata.split("#PIECES")[1].split("\n"))
                          )
    ...
    while recent:
        ...
        for i in range(len(plist)):
            if plist[i][0].split(".")[0] == "routine":
                recent = True
                # TOCLEAN: path.join()
                schpath = str(cconf.schematics_folder)+"/"+str(plist[i][0].split(".")[1])+'.schematic'
                tf = open(schpath, 'r')
                temp = tf.read()
                tf.close()
